...
export default function ImagePage() {
    ...
    // 同视频页：没有可用图片模型时不放行，避免请求发出去才失败且看不出原因。
    const hasImageModel = Boolean(model) && selectableModelsByCapability(effectiveConfig, "image").length > 0;
    const canGenerate = Boolean(prompt.trim()) && hasImageModel;
